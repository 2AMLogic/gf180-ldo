...
- **Status**: **ratified 2026-09-15** — by the operator's approval and merge
  of pull request #137 (issue #105), per the 2026-08-19 ratification-via-PR
  policy (2AMLogic/2am#357). The two-key mechanism (2AMLogic/2am#372) ran on
  this PR 2026-08-21: EE key `approve`, market key `adequate-for-catalog`
  (current-limit row `competitive`; see §Operator ruling below). Supersedes
  DR-0004's Current limit and Thermal rows only; `README.md`'s rows carry the
  ratified 62–95 mA / ≤ 346 mW untrimmed window.
- **Date**: 2026-08-01
...
**Operator ruling: ratify.** Both keys approved this record on 2026-08-21.
The EE key found that both changed rows re-derive independently from the
cited design source and the fresh 63-point record
(`sim/current-limit/records/20260816-080440-8e105a0.md`), that the
physics claim — the window is set by one uncancelled absolute `ppolyf_u`
resistance whose sheet spread alone moves the limit ±20 % — is corroborated
directly in the netlist, and that the 346 mW thermal ceiling holds as
`P = I × V` at every corner. The market key ran the relax-after-measured-FAIL
check and found the untrimmed 62–95 mA window **proportionally tighter than
every public untrimmed comp it surveyed** (five shipping small-package LDOs
from three vendors publish current-limit min/max spreads of 2.25×–3.9×
against this record's 1.52–1.53×, and two of the five publish no guaranteed
floor at all); the one tighter listed part reaches ±10 % only through an
external customer-set trim resistor, which is the same mechanism this record
names as the only route to ±10 %, so it corroborates the premise rather than
contradicting it. The thermal row was rated adequate-for-catalog: the 346 mW
ceiling is a correctly derived consequence of the current-limit row, and the
absence of an on-chip thermal shutdown is disclosed in `README.md` note 5
rather than claimed away.

The operator's finding follows: the 65–80 mA window and its ≤ 290 mW
consequence were ratified before any PVT sweep existed and are structurally
unreachable untrimmed on this PDK; amending them to the measured envelope
corrects a provisional row against its own falsifiable trigger (DR-0004),
which is what that trigger was for. No design change is made, no other row is
touched, and the ±10 % window stays a stretch goal conditional on trim. The
mechanical consequence recorded above stands: the current record and
`sim/CHARACTERIZATION.md` keep reading FAIL against the superseded bounds
until a follow-on record states its verdict against 62–95 mA / 346 mW.
