...
- **Status**: **ratified 2026-09-14** — by the operator's approval and merge
  of pull request #127 (issue #106), per the 2026-08-19 ratification-via-PR
  policy (2AMLogic/2am#357). The two-key mechanism (2AMLogic/2am#372) ran on
  this PR 2026-08-27: EE key `approve`, market key `escalate` (relax-after-
  measured-FAIL rule; see §Market-key finding below), so release required an
  explicit operator ruling, given 2026-09-14.
- **Date**: 2026-08-01
...
Because this record weakens a previously-ratified value (3 ms → 6 ms), FLEET.md's
relax-after-measured-FAIL rule required the market key to find 6 ms competitive
against named public parts before autonomous release. It could not: the one
directly comparable listed part it found (TI TLV700, already in the LDO comp
library for other rows) specifies a start-up time roughly 60× faster, so it
escalated rather than ruling `competitive`.

**Operator ruling: approve 6 ms.** The 3 ms settling bound and the ≤ 1 V/ms ramp
bound were ratified together in DR-0004 but jointly admit only a 1.70 : 1 window,
which no untrimmed on-chip ramp meets across PVT (the arithmetic in §Context).
The 3 ms value was therefore a specification error, not a design target the
circuit fell short of, and amending it to the measured envelope corrects the
spec rather than relaxing it to fit a result. The competitiveness finding stands
and is carried as a disclosed gap: this block's startup time is not competitive
with fast-start public LDOs, and any catalog claim must say so. Closing that gap
(a faster ramp with an inrush budget to match, or a trimmed ramp) is a separate
design decision, not something this record decides.
