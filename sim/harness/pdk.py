...
This is the same resolution order (and the same env var names) as
2AMLogic/gf180-bandgap's harness, deliberately -- cross-block tooling and
review uniformity is the point of matching (see sim/README.md's provenance
note). ``design/xschemrc`` (once #8 lands schematic entry) must resolve the
PDK by this same order so xschem and this harness never disagree about
which install is in use.
"""
