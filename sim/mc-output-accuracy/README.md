...
```
python3 sim/mc-output-accuracy/summarize.py 20260816-141924-9f85f6b
```
...
- **NOT simulated, and cannot be against this PDK:** the feedback divider.
  ...
  under 0.0043 µV (4.3 nV), ~230× smaller than the 1 µV integrity ceiling the
  check imposes — confirming the divider ratio held exactly and this run's
  spread is the amplifier's alone.
...
- **Record**: `sim/mc-output-accuracy/records/20260816-141924-9f85f6b.md` —
  **PASS**, 45/45 points, every check (`n_samples`, `n_bad_solves`,
  `sigma_mv` integrity floor, `sigma3_mv`, `vos_in_sigma3_mv`, `worst_abs_mv`,
  `divider_exact_uv`, `mean_mv`) passing at every corner. Run against
  `design/netlist/ldo_core.spice` sha256 `3aeb2259…`, i.e. the current
  post-`Cff` (#42) / `Xilimit` (#11) / `Xsoftstart` (#38) / `Mrza`+`Rza`
  (DR-0015, #78) netlist. **Supersedes** `20260801-030232-969e423`, which was
  taken against the pre-#11 `ldo_core` and is stale for all four landings
  (#103, from #95's T1/bronze re-read).
- **Raw per-corner logs** (including the per-sample `MCSAMPLE` echoes this
  rollup reads): `sim/mc-output-accuracy/corners/20260816-141924-9f85f6b/`
- **Frozen netlist**: `sim/mc-output-accuracy/netlist-snapshots/20260816-141924-9f85f6b.spice`
...
- **Worst sigma**: `ss_125c_2.97v` — σ = 0.741 mV, 3σ = **2.223 mV** output /
  **1.482 mV** input-referred. This is the number checked against §3.1's
  calculated target below.
- **Worst |mean|**: `sf_125c_3.63v` — mean = **−1.175 mV**, inside the
  deterministic ±2.56 mV budget line at every one of the 45 points.
- **Worst single sample**: `fs_125c_3.63v` — |ΔVout| = 3.578 mV, far inside
...
Largest \|z\| observed: 4.24σ over 45,000 samples (expected order of
...
|---|---|---|---|
| Amp input-pair + mirror mismatch | **2.22 mV** (3σ) | statistical | **this run**, worst corner `ss_125c_2.97v` |
...
| (same, as measured by this run's worst per-corner mean) | 1.17 mV | deterministic | **this run**, `sf_125c_3.63v` |
| **Statistical RSS** | **4.03 mV** | | |
| **Total static (ratified deterministic)** | **6.59 mV** | | vs 18 mV available |
| **Total static (measured deterministic)** | **5.20 mV** | | |
| Load regulation (allowance, #12 verifies) | 18.0 mV | | |
| **Total against the ±36 mV window** | **24.59 mV** | | **32 % margin** |

The measured amp-mismatch term (2.22 mV) beats §3.1's calculated 3.49 mV
target, so this run's total-static-vs-window margin (32 %) is slightly better
than §3's calculated 29 %.

### Yield

- Combined statistical 1σ (amp RSS divider): 1.343 mV
- Headroom after the deterministic terms and the load-regulation allowance:
  36 − 18 − 2.56 = 15.44 mV
- That headroom is **11.5σ** of the combined statistical distribution →
...
|---|---|---|
| Measured (this run, worst corner) | **2.22 mV** | **1.48 mV** |
| §3.1 calculated target | 3.49 mV | 2.33 mV |
| Measured / calculated | 0.637 | 0.637 |
...
Total against the ±36 mV window: 24.59 mV → **32 % margin**. No `spec/`
